...
import { getAllEvents, getAllReadings } from "../../services/edgexApi";
...
function DataCenter() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const uniqueDevices = Array.from(new Set(items.map((ev) => ev.deviceName)));
  ...
  const fetchReadings = async () => {
    try {
      const { data } = await getAllReadings(0, 10);
      const items = data.readings || [];
      setReadingsRaw(items);

      const uniqueDevices = Array.from(new Set(items.map((rd) => rd.deviceName)));
      setDevices(uniqueDevices);
    } catch (err) {
  ...
  const renderFilterDropdown = () => (
    <div
      className="filter-panel mb-3 p-3 border rounded bg-light"
  ...
  return (
    <div className="datacenter container mt-4">
      ...
      <div className="section-tabs mb-3">
        <button
          type="button"
          className={`section-tab-btn ${activeTab === "event" ? "active" : ""
            }`}
        ...
        <button
          type="button"
          className={`section-tab-btn ${activeTab === "reading" ? "active" : ""
            }`}
